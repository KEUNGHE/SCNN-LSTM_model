#A code file for implementing the SCNN-LSTM model
library(keras)
...
for(kk in 1:12){
  ...
  for(i in 1:10){
    ...
    save_model_hdf5(model_scnnlstm,paste0("CSNN-LSTM_",mark[kk],"_",i,".h5"))
  ...
  assign(paste0("CSNN_",mark[kk]),matr)
  write.csv(get(paste0("CSNN_",mark[kk])),paste0("CSNN_",mark[kk],".csv"))
}
